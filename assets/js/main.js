...
(function ($) {
  ...
  function addtocart_popup() {
    $('.pro-addtocart-popup').on('click', function (e) {
      e.preventDefault();
      let productName = $(this).parents('.item').find('.product-name a').text();
      Toastify({
        text: `${productName} به سبد خرید اضافه شد`,
        duration: 3000,
        newWindow: true,
        close: true,
  ...
  // image Preview
  $('[data-addimage]').change(function (e) {
    e.preventDefault();
    let img = e.target.files[0];
    let imgSrc = URL.createObjectURL(img);

    $(this).parents('.form-group').find('img').attr('src', imgSrc);
  });

  // show form New Ticket
  $('.btnAddNewTicket').click(function (e) {
    e.preventDefault();
    $('.add-new-ticket').fadeIn('slow');
  });

  // dismiss form Ticket
  $('.btn-cancel').click(function (e) {
    e.preventDefault();
    $('.add-new-ticket').fadeOut('slow');
  });
})(jQuery);
